...
## no forbidden squares 
#forbidden = set([]) 
## middle row forbidden
...
def getAdjacencies(k): 
  ...
  if c < w - 1: 
    adjacencies.append(k + 1)
  # additional edges not on grid
  if r == 0 and c == 0: 
    adjacencies.append(w + 1)
  if r == 1 and c == 1:
    adjacencies.append(0)
  # probably check that h and w are not too small 
  if r == h-1 and c == w-1:
    adjacencies.append(h * w - 2 - w)
  if r == h-2 and c == w-2: 
    adjacencies.append(h * w - 1)
  return adjacencies
